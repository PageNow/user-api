...
WITH friends AS (
    ...
    SELECT user_id, MAX(match_score) AS match_score FROM (
        SELECT user_id, 3 AS match_score FROM user_table WHERE email LIKE 'm%'
        UNION
        SELECT user_id, 2 AS match_score FROM user_table WHERE email LIKE '%m'
        UNION
        SELECT user_id, 1 AS match_score FROM user_table WHERE email LIKE '%m%'
    ) u GROUP BY user_id
), friends_accepted AS (
    SELECT user_id1, user_id2 FROM friendship_table WHERE accepted_at IS NOT NULL
), user_friend_accepted AS (
    SELECT user_id FROM friends WHERE accepted_state = 2    
), search_result AS (
    ...
    SELECT user_id, first_name, last_name, description, profile_image_extension,
        accepted_state, match_score, friends_accepted.user_id2 AS friend_id
    FROM search_result LEFT JOIN friends_accepted ON user_id = user_id1
    UNION
    SELECT user_id, first_name, last_name, description, profile_image_extension,
        accepted_state, match_score, friends_accepted.user_id1 AS friend_id
    FROM search_result LEFT JOIN friends_accepted ON user_id = user_id2
